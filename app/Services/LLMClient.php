...
class LLMClient
{
    ...
    private function requestWithRetry(string $url, array $body, array $headers, int $timeout, int $maxRetries = 2): ?\Illuminate\Http\Client\Response
    {
        ...
        for ($attempt = 0; $attempt <= $maxRetries; $attempt++) {
            try {
                ...
                Log::warning('LLMClient connection error', [
                    'error' => $e->getMessage(),
                    'attempt' => $attempt + 1,
                ]);
        ...
        try {
            ...
            if (!$result->successful()) {
                ...
                if ($data && ($data['subtype'] ?? '') === 'error_max_turns') {
                    Log::info('LLMClient::executeClaudeCli max_turns hit', [
                    ...
                    Log::warning('LLMClient::executeClaudeCli failed', [
                    ...
                    if ($sessionId) {
                        Log::info('LLMClient: session resume failed, starting fresh');
            ...
            if (!$data) {
                Log::warning('LLMClient::executeClaudeCli invalid JSON', [
            ...
            if (($data['is_error'] ?? false) && empty($data['result'])) {
                Log::warning('LLMClient::executeClaudeCli error', [
            ...
            Log::error('LLMClient::executeClaudeCli exception', ['error' => $e->getMessage()]);
            return null;
        } finally {
            // Clean up temp files
    ...
    public function chat(string|array $message, string $model = 'claude-haiku-4-5-20251001', string $systemPrompt = '', int $maxTokens = 0, bool $forceApi = false): ?string
    {
        ...
        if (!$this->isCircuitClosed()) {
            ...
            if ($fallback && $fallback !== $model) {
                Log::info('LLMClient: circuit open, trying fallback', ['from' => $model, 'to' => $fallback]);
                $model = $fallback;
            } else {
                Log::warning('LLMClient: circuit open, no fallback available');
                return null;
            }
        }
        ...
        for ($cont = 0; $cont <= $maxContinuations; $cont++) {
            ...
            if (!$response || !$response->successful()) {
                ...
                if ($cont === 0 && $response) {
                    ...
                    if ($error['type'] === 'rate_limit' || $error['type'] === 'overloaded') {
                        $fallback = $this->getFallbackModel($model);
                        if ($fallback && $fallback !== $model) {
                            Log::info('LLMClient: falling back model', ['from' => $model, 'to' => $fallback]);
                            $body['model'] = $fallback;
            ...
            Log::info('LLMClient::chat continuation needed', [
                'continuation' => $cont + 1,
    ...
    public function chatWithToolUse(array $messages, string $model, string $systemPrompt = '', array $tools = [], int $maxTokens = 4096): ?array
    {
        ...
        if ($response) {
            ...
            if ($error['type'] === 'rate_limit' || $error['type'] === 'overloaded') {
                $fallback = $this->getFallbackModel($model);
                if ($fallback && $fallback !== $model) {
                    Log::info('LLMClient::chatWithToolUse fallback', ['from' => $model, 'to' => $fallback]);
                    $body['model'] = $fallback;
        ...
        Log::error('LLMClient::chatWithToolUse failed', [
            'status' => $response?->status(),
    ...
    public function chatWithMessages(array $messages, string $model, string $systemPrompt = '', int $maxTokens = 4096): ?string
    {
        ...
        Log::error('LLMClient::chatWithMessages failed', [
            'status' => $response?->status(),
            'body' => $response ? substr($response->body(), 0, 500) : 'no response',
            'model' => $model,
        ]);
